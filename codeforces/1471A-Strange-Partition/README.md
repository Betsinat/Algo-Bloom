...
**Problem:** [1471A-Strange-Partition](https://codeforces.com/contest/1471/problem/A)
...
The beauty of an array b=[b_1, …, b_k] is defined as ∑_{i=1}^k ≤ft\lceil \frac{b_i}{x} \right\rceil, which means that we divide each element by x, round it up to the nearest integer, and sum up the resulting values. For example, if x = 3, and the array is [4, 11, 6], the beauty of the array is equal to ≤ft\lceil \frac{4}{3} \right\rceil + ≤ft\lceil \frac{11}{3} \right\rceil + ≤ft\lceil \frac{6}{3} \right\rceil = 2 + 4 + 2 = 8.
...
The first line of each test case contains two integers n and x (1 ≤q n ≤q 10⁵, 1 ≤q x ≤q 10⁹).

The next line contains n integers a_1, a_2, …, a_n (1 ≤q a_i ≤q 10⁹), the elements of the array a. 

It is guaranteed that the sum of values of n over all test cases does not exceed 10⁵.
